...
class StacksTestJSON(base_multipleusers.BaseMultipleOrchestrationTest):
    ...
    def test_updatestack_limit(self):
        ...
        pattern = load_pattern.ConstantLoad(1)
        runner = LoadTestRunner(pattern)
        runner.run(self._update_stack)

    def test_createstack_limit(self):
        #limit 20/min
       # pattern = load_pattern.StepLoad(2, 20, 2, 2)
        pattern = load_pattern.ConstantLoad(3)
        runner = LoadTestRunner(pattern)
        runner.run(self._create_stack)
    ...
    def _get_stack(self):
        ...
        resp, stacks = self.list_stack(user_first)
        list_ids = list([stack['id'] for stack in stacks])
        print list_ids
        print datetime.datetime.now()
        user_second = self.managers[1]
        resp, stacks = self.list_stack(user_second)
        list_ids = list([stack['id'] for stack in stacks])
        print list_ids
        print datetime.datetime.now()
        print "OVER GET STACK CALL"
    ...
    def _update_stack(self):
        ...
        self._create_stack()
        for i in range(1):
            resp, stacks = self.list_stacks(user_first)
            for stack in stacks:
                stack_name = stack['stack_name']
                stack_id = stack['id']
                resp, stack = self.update_stack(stack_id,stack_name)
                print resp
            print datetime.datetime.now()
    ...
    def _create_stack(self):
        ...
        print "Creating for user1"
        stack_identifier = self.create_stack(user_first,stack_name, region,
                                             yaml_template, parameters)
        print stack_identifier

        stack_id = stack_identifier.split('/')[1]

        print "Creating for user2"
        stack_identifier_2 = self.create_stack(user_second,stack_name, region,
                                             yaml_template, parameters)
        print stack_identifier_2

       # stack_id2 = stack_identifier_2.split('/')[1]
